...
/**
 * Returns the query scope that represents geolocalisations accessible by the
...
exports.getOwnerScope = req => {
  ...
  if (user.hasRole('owner_admin') || user.hasRole('owner_validator')) {
    // An owner administrator or validator can only update geolocalisations on images
